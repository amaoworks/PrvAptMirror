...
本文定义 PrvAptMirror 第二阶段的部署体验改造。目标不是把所有进程塞进一个容器，而是把多个安全边界封装成一个 Compose 项目、一个数据根目录和一套统一命令，让日常使用者不需要理解每个内部容器。
...
相对路径由统一脚本以项目根目录为基准解析并转换为绝对路径后再交给 Compose，不能沿用 Compose 文件所在目录的隐式相对路径规则。
...
1. 引入 `PRVAPTMIRROR_DATA_DIR` 并把全部应用绑定挂载收敛到该目录；
2. 增加幂等 `bootstrap` 服务和数据布局版本；
3. 自动初始化或复用 GPG 密钥并导出公钥；
4. 实现一次性设置令牌和浏览器首次密码设置页面；
5. 把正常启动收敛为完整多容器 Compose 编排；
6. 统一 `up`、`stop`、`status`、`logs`、`setup` 和高级 `repoctl` 命令；
7. 为重复初始化、令牌攻击、密码原子写入、目录权限、容器挂载和无网络 Worker 增加测试；
8. 提供从当前 `var/` 布局迁移到新数据根目录的说明和检查命令。
...
5. 当前 `var/` 数据只做显式迁移，不自动移动，不自动删除，也不在迁移前修改；
6. 本批次止于统一部署和首次设置，不同时实现上传、Worker 业务任务、发布或回滚页面。
