...
第一阶段的命令行仓库基础已经完成。当前第二阶段增加单管理员 Web 管理，但保持公开下载、管理入口和签名执行三个权限域相互隔离。
...
`admin-web` 提供单管理员登录、管理页面、上传、只读查询以及结构化任务创建。它只能访问管理状态、上传和任务目录，不能访问 Docker Socket、GPG 私钥、Aptly 数据库或公开仓库写权限。

认证使用 Argon2id 密码哈希和服务端会话，不保存明文密码。状态变更请求必须具有 CSRF 防护、来源检查、登录限速和审计记录。详细约束参见 [单管理员 Web 管理](web-admin.md)。
...
`repo-web` 是非特权静态文件容器，只读挂载 `var/public` 并向公开反向代理提供内部 HTTP 端口。它没有上传接口、管理 API、任务目录、Aptly 数据库或签名私钥。

## 持久化数据

| 路径 | 写入者 | 读取者 | 内容 |
| --- | --- | --- | --- |
| `var/incoming` | 现有 SSH 流程、第二阶段上传流程 | `repoctl`/Worker | 等待导入的软件包 |
| `var/admin/uploads` | `admin-web` | `admin-web`、Worker | 上传临时文件、校验结果和待处理 DEB |
| `var/admin/jobs` | `admin-web`、Worker | `admin-web`、Worker | 结构化任务、状态和脱敏结果 |
| `var/admin/audit` | `admin-web`、Worker | `admin-web` | 追加式管理审计日志 |
| `var/lib/aptly` | `repoctl`/Worker | `repoctl`/Worker | Aptly 数据库、包池、快照和发布状态 |
| `var/lib/gnupg` | `repoctl`/Worker | `repoctl`/Worker | 仓库签名私钥环 |
| `var/public` | `repoctl`/Worker | `repo-web` | 已签名的公开 APT 目录和公钥 |
